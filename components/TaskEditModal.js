...
const TaskEditModal = ({ visible, onClose, onEdit, onDelete, task }) => {
  ...
  const handleEdit = () => {
    if (task && (editedTask.trim() !== "" || editedTime.trim() !== "")) {
      onEdit({ ...task, task: editedTask, time: editedTime });
      setEditedTask("");
      setEditedTime("");
      onClose();
    }
  ...
  return (
    <Modal visible={visible} animationType="slide">
      <View className="flex-1 justify-center items-center bg-purple-200  p-4 border-solid border-8 border-yellow-200">
        <Text className="p-2 mb-4 w-full text-md text-center">
          Edit or Delete here!🤠
        </Text>
        <TextInput
        ...
        <TouchableOpacity onPress={onClose} className="mt-4">
          <Text className="bg-purple-400 p-3 rounded-lg text-xl text-center w-24">
            Close
          </Text>
        </TouchableOpacity>
      </View>
